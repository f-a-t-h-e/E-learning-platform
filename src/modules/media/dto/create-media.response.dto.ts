...
import { IsDate, IsInt, IsString, IsUrl, IsEnum, IsPositive } from 'class-validator';
...
export class CreateMediaResponseDto implements Media {
    ...
    @ApiProperty({ description: 'The ID of the course associated with the media' })
    @IsPositive()
    @IsInt()
    courseId: number;
    ...
    @ApiProperty({ description: 'The ID of the lesson associated with the media' })
    @IsPositive()
    @IsInt()
    lessonId: number;

    @ApiProperty({ description: 'The state of the media', enum: $Enums.MediaState })
